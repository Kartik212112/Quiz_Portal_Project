...
def Register():
    ...
    btnEnter_2 = Button(
        ...
        relief = FLAT,
        border = 0,
        command = lambda: EnterRegister(entry_roll_no,entry_password,entry_user_name,entry_whatsapp)
        )
    btnEnter_2.pack()
...
def submit():
    ...
    for row in c.execute('SELECT * FROM project1_marks'):
        if row[0] == roll_no and row[1] == quiz_no:
            conn.execute("DELETE FROM project1_marks WHERE roll = ? AND quiz_num = ?",(roll_no, quiz_no))
            conn.execute("INSERT INTO project1_marks VALUES(?,?,?);", (roll_no, quiz_no, total_marks))
            break    
    else:
         conn.execute("INSERT INTO project1_marks VALUES(?,?,?);", (roll_no, quiz_no, total_marks))
...
def quiz_unpack(var):
    ...
    individual_df.drop(individual_df.columns[len(individual_df.columns)-1], inplace=True, axis=1)
...
def export_ask(event):
    ...
    MsgBox = tkMessageBox.askquestion ('Export','Do you want to export to csv?',icon = 'info')
    if MsgBox == 'yes':

        marksrecord_df = pd.read_sql_query("SELECT * FROM project1_marks", conn)
        ...
        for i in range(1, 4):
            ...
            quizrecord_df = quizrecord_df.reindex(columns = ['Name','Whatsapp'] + quizrecord_df.columns.tolist())
...
def goto_ask(event):
    global quiz_no
    ques_no = int(simpledialog.askstring(title="Goto",prompt="Go to question no.: "))
